...
const processTimer = () => {
  ...
  startTimerBtn.classList.toggle(
  'hidden');
...
const randomInt = (min = 0, max = 16) => Math.round(Math.random() * (max - min) + min);
...
In our example above, the remainder is 14, which means we still stay at the 6th slice from the original slice.
  */
  ...
  const finalSlice = document.querySelector(`.slice__container[data-order="${finalSliceOrder}"]`);
...
spinBtn.addEventListener('click', () => {
  ...
  const randDeg = startingDeg + Math.round(randomInt(10, 20) * 360 + randomInt(16, 316));
